...
object SQLOnSpark {
  def main(args: Array[String]) {
    ...
    val people: RDD[Person] = sc.textFile("hdfs://219.219.220.149:9000/xubo/spark/dataSQL/people.txt").map(_.split(",")).map(p => Person(p(0), p(1).trim.toInt))
    //    val people: RDD[Person] = sc.textFile("data/people.txt").map(_.split(",")).map(p => Person(p(0), p(1).trim.toInt))
    println(people);
    for (i <- people) {
    ...
    val parquetFile = sqlContext.read.parquet("hdfs://219.219.220.149:9000/xubo/spark/dataSQL/wiki_parquet")
    //parquet：second method to reads from parquet file
    //    val parquetFile = sqlContext.parquetFile("hdfs://219.219.220.149:9000/xubo/spark/dataSQL/wiki_parquet")
    println(parquetFile);
