...
object SparkSQLReadParquetMerge {
  def main(args: Array[String]) {
    ...
    df1.write.parquet("file/data/" + iString + "/test_table/key=1/")
    df1.show
    
    // Create another DataFrame in a new partition directory,
    // adding a new column and dropping an existing column
    val df2 = sc.makeRDD(6 to 10).map(i => (i, i * 3)).toDF("single", "triple")
    df2.write.parquet("file/data/" + iString + "/test_table/key=2/")
    df2.show
    
    // Read the partitioned table
    val df3 = sqlContext.read.option("mergeSchema", "true").parquet("file/data/" + iString + "/test_table/")
    df3.printSchema()
    df3.show
