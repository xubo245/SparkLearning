...
object KMeansTest3 {
  def main(args: Array[String]) {
    ...
    val conf = new SparkConf().setAppName("KMeansTest3").setMaster("spark://219.219.220.149:7077")
    val sc = new SparkContext(conf)

    // 装载数据集
    val data = sc.textFile("hdfs://219.219.220.149:9000/xubo/spark/data/mllib/kmeans_data.txt", 1)
    ...
    result1.saveAsTextFile("hdfs://219.219.220.149:9000/xubo/spark/output/mllib/kmeans_data/"+iString+"/1")
    ...
    val result2 = data.map {
      line =>
        ...
        line + " " + prediction
    }.saveAsTextFile("hdfs://219.219.220.149:9000/xubo/spark/output/mllib/kmeans_data/"+iString+"/2")
    sc.stop()
  }
}