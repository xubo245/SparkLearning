 ...
 */
package org.apache.spark.mllib.learning.kmeans
  ...
  def main(args: Array[String]) {
    ...
    val result1 = parsedTrainingData.map {
      line =>
        ...
        line + " " + prediction
    }.saveAsTextFile("file/data/mllib/output/KMeansTest4ByIBmSaveAsFileCluster/" + iString + "/train")
    ...
    val result2 = parsedTestData.map {
      line =>
        val prediction = clusters.predict(line)
        line + " " + prediction
    }.saveAsTextFile("file/data/mllib/output/KMeansTest4ByIBmSaveAsFileCluster/" + iString + "/test")
