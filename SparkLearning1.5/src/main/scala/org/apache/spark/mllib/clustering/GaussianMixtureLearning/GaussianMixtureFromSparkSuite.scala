...
class GaussianMixtureFromSparkSuite extends SparkLearningFunSuite {
  test("single cluster") {
  ...
  test("two clusters with sparse data") {
    ...
    /** ****************add by xubo 20160613 *************/
    for (i <- 0 until sparseGMM.k) {
      println("weight=%f\nmu=%s\nsigma=\n%s\n" format
        (sparseGMM.weights(i), sparseGMM.gaussians(i).mu, sparseGMM.gaussians(i).sigma))
    }
